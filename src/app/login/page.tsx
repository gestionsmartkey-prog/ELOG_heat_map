  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-100 px-4">
      <form method="post" action="/api/login" className="w-full max-w-sm rounded-xl bg-white p-8 shadow-md border border-slate-200">
        ...
        <label className="mt-6 block text-sm font-medium text-slate-700" htmlFor="password">Password</label>
        <input
          id="password"
          name="password"
          type="password"
        ...
        />
        {error ? <p className="mt-2 text-sm text-red-600" role="alert">Wrong password. Try again.</p> : null}
