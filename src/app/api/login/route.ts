...
import { SESSION_COOKIE, checkEnvCredentials, createSessionToken, sessionCookieOptions } from "@/lib/auth";
...
export async function POST(request: Request) {
  ...
  // Env users (bootstrap admins + viewers + shared fallback) first, then the whitelist table.
  const session = checkEnvCredentials(username, password) ?? (await verifyDbUser(username, password));
  if (!session) {
