...
const PASS_LIMIT = 5;          // locations per database call; each one is a few georef HTTP round-trips
...
export async function runGeocodePass(opts: { retry?: boolean; budgetMs?: number } = {}): Promise<GeocodePass> {
  ...
  for (;;) {
    const { data, error } = await sb.rpc("georef_geocode_pending", { p_limit: PASS_LIMIT, p_retry: retry });
    if (error) throw new Error(`georef_geocode_pending: ${error.message}`);
    ...
    if (retry && n < PASS_LIMIT) retry = false;
